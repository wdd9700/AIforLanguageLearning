/**
 * @fileoverview 服务抽象层 (Service Abstraction Layer)
 * @description
 * 该模块定义了服务的基类和具体实现，用于统一管理不同类型的服务（进程服务、HTTP 服务）。
 * 
 * 主要类包括：
 * 1. BaseService (抽象基类)：
 *    - 定义了服务的通用接口 (start, stop, healthCheck, invoke)
 *    - 实现了状态管理和健康检查循环
 *    - 处理自动重启逻辑
 * 
 * 2. ProcessService (进程服务)：
 *    - 管理本地子进程 (spawn, kill)
 *    - 监控 stdout/stderr 日志
 *    - 检查进程存活状态
 * 
 * 3. HttpService (HTTP 服务)：
 *    - 管理远程 HTTP 连接
 *    - 实现了特定的 API 调用逻辑 (如 LLM 适配)
 *    - 支持请求超时和中止
 * 
 * 4. createService (工厂函数)：
 *    - 根据配置创建相应的服务实例
 * 
...
export abstract class BaseService {
  ...
  protected startHealthChecking(): void {
    ...
    this.healthCheckTimer = setInterval(async () => {
      try {
        if (this.instance.status !== ServiceStatus.RUNNING) return;
   ...
   * 处理健康检查失败
   */
  protected async handleHealthCheckFailure(): Promise<void> {
...
export class ProcessService extends BaseService {
  ...
  async invoke(method: string, params: any, timeout?: number): Promise<any> {
 ...
 * 服务工厂
 */
export function createService(config: ServiceConfig): BaseService {
  switch (config.type) {
    case 'process':
